...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place and current_place is not hive and i != self.max_range:
            if i >= self.min_range and current_place.bees:
                return random_or_none(current_place.bees)
            current_place = current_place.entrance
            i += 1
        return None
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt doesn't do anything but stand there till to dead."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is water safe thrower."""
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            dinner = random_or_none(self.place.bees)
            self.eat_bee(dinner)
        # END Problem 10
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        def queen_behind(self):
            ants, place = [], self.place.exit
            while place:
                if place.ant:
    ...
    def reduce_armor(self, amount):
        ...
        if not self.is_queen:
            ScubaThrower.reduce_armor(self, 0)
        else:
            self.armor -= amount
            if self.armor <= 0:
                bees_win()
